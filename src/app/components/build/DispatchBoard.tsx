    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[12.5px] font-medium text-[#000000] truncate leading-tight">{job.customer}</p>
          <p className="text-[11px] text-[#636363] truncate mt-0.5">{job.jobType}</p>
        </div>
        {draggable && <GripVertical className="w-3.5 h-3.5 text-[#D9D9D9] flex-shrink-0 group-hover:text-[#959595]" />}
      </div>

      {!compact && (
        <div className="flex items-center gap-1 mt-2 text-[10.5px] text-[#959595]">
      ...
      <div className="flex items-center gap-1.5 mt-2 flex-wrap">
        ...
        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium"
          style={{ background: job.overdue ? '#FBEAEA' : '#F0F0F0', color: job.overdue ? '#E5484D' : '#636363' }}>
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: '#F8F8F8' }}>
      ...
      <div className="flex items-center gap-2.5 px-4 h-14 flex-shrink-0">
        ...
        <div className="min-w-0">
          <p className="text-[15px] font-medium tracking-[-0.015em] text-[#000000]">Quote follow-up</p>
          <p className="text-[11px] text-[#959595]">Status triage · review before send</p>
        </div>
        <button className="inline-flex items-center gap-2 h-8 pl-3 pr-2 rounded-full border border-[rgba(0,0,0,0.08)] bg-white hover:border-[#000000] transition-colors text-[11.5px] font-mono text-[#636363] ml-2">
          <SlidersHorizontal className="w-3 h-3 text-[#959595]" />
          TX · HVAC · this week
          <ChevronDown className="w-3 h-3 text-[#959595]" />
        </button>
        <span className="ml-auto text-[11.5px] text-[#959595]">
          <span className="font-medium text-[#636363]">{tray.length}</span> unassigned · <span className="font-medium text-[#636363]">{techs.length}</span> techs
      ...
      <div data-app-reveal className="flex-1 flex gap-3 p-3 pt-0 overflow-x-auto" style={{ background: '#F8F8F8' }}>
        ...
        <div className="w-[240px] flex-shrink-0 flex flex-col rounded-[24px] bg-white border border-white p-2"
          ...
          <div className="flex-1 flex flex-col gap-2 overflow-y-auto rounded-[18px] bg-[#F8F8F8] border border-[rgba(0,0,0,0.05)] p-2">
            {tray.map(job => (
            ...
            {tray.length === 0 && (
              <div className="flex-1 flex items-center justify-center text-[11.5px] text-[#959595] border border-dashed border-[rgba(0,0,0,0.08)] rounded-[18px] py-8">
                All jobs assigned
              </div>
            )}
          </div>
          ...
          return (
            <div key={tech.id} className="w-[240px] flex-shrink-0 flex flex-col rounded-[24px] bg-white border border-white p-2"
              ...
              <div className="flex items-center gap-2 px-1 pb-2.5 h-6">
                ...
                <span className="text-[12px] font-medium text-[#636363] truncate">{tech.name}</span>
                <span className="ml-auto text-[10.5px] font-mono text-[#D9D9D9]">{tech.jobIds.length}</span>
              </div>
              <div
                ...
                style={{
                  background: isOver ? (matches ? '#F0F0F0' : '#F0F0F0') : '#F8F8F8',
                  border: isOver ? `1.5px dashed ${matches ? '#000000' : '#000000'}` : '1px solid rgba(0,0,0,0.05)',
                  minHeight: 120,
                }}
              >
                {dragId && isOver && (
                  <div className="text-[10.5px] font-medium text-center py-1.5 rounded-md"
                    style={{ background: matches ? '#F0F0F0' : '#FFE2CC', color: matches ? '#000000' : '#C2410C' }}>
                ...
                {tech.jobIds.length === 0 && !isOver && (
                  <div className="flex-1 flex items-center justify-center text-[11px] text-[#D9D9D9] py-6">Drop jobs here</div>
                )}
              </div>
            </div>
          );
            ...
            >
              <div className="flex items-center gap-2.5 mb-3">
                <div className="w-9 h-9 rounded-full bg-[#F0F0F0] flex items-center justify-center flex-shrink-0">
                  <ShieldCheck className="w-4.5 h-4.5 text-[#000000]" />
                </div>
                <div>
                  <h3 className="text-[15px] font-medium text-[#000000]">Confirm write</h3>
                  <p className="text-[11.5px] text-[#959595]">Previewed before running · audited</p>
                </div>
              </div>
              <div className="rounded-xl bg-[#FAFAFA] border border-[rgba(0,0,0,0.08)] p-3 text-[12.5px] space-y-1.5">
              ...
              <button onClick={() => setDryRun(v => !v)}
                ...
                style={{ background: dryRun ? '#F0F0F0' : '#F8F9FB' }}>
                <span className="w-4 h-4 rounded flex items-center justify-center flex-shrink-0" style={{ background: dryRun ? '#000000' : '#FFF', border: dryRun ? 'none' : '1.5px solid #D1D5DB' }}>
                  {dryRun && <Check className="w-3 h-3 text-white" />}
                </span>
                <span className="text-[11.5px] text-[#636363]">Dry-run — validate the write without changing records</span>
              </button>
              <div className="flex items-center gap-2 mt-4">
                <button onClick={() => { setConfirm(null); setDryRun(false); }}
                  className="flex-1 h-9 rounded-xl text-[13px] font-medium text-[#636363] bg-[#F0F0F0] hover:bg-[#E9EBEE] transition-colors">
                  Cancel
                </button>
                <button onClick={commitAssign}
                  className="flex-1 h-9 rounded-xl text-[13px] font-medium text-white bg-[#000000] hover:bg-[#1A1A1A] transition-colors inline-flex items-center justify-center gap-1.5">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[420] flex items-center gap-2 px-4 h-10 rounded-full bg-[#000000] text-white text-[12.5px] font-medium"
...
function Row({ k, v, mono }: { k: string; v: string; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <span className="text-[#959595]">{k}</span>
      <span className={`text-[#000000] font-medium text-right ${mono ? 'font-mono text-[11.5px]' : ''}`}>{v}</span>
    </div>
  );
}
